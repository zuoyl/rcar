...
DECL_RESOURCE_CLASS(SellerCommentList, "user/seller-comment-list");

int SellerCommentList::Get(const ResourceContext &context, Request &req) {
    ...
    try{
        ...
        if (cursor->more()) {
            ...
            for (int i = (page*page_size); (i < com_bson.nFields()) && (count < page_size); i++) {
                ...
                SellerCommentList.append(comment);
                count++;
            }            
        }
        out["status"] = APIE_OK;
        out["total"] = count;
        out["data"] = SellerCommentList;
        return APIE_OK;
    }
...
int SellerCommentList::Post(const ResourceContext &context, Request &req) {
...
#endif
        // update
        BSONObj info = BSON("$push" <<  BSON("SellerCommentList" << comment_obj.obj()));
...
int SellerCommentList::Put(const ResourceContext &context, Request &req) { 
    return Resource::Put(context, req);
}

int SellerCommentList::Delete(const ResourceContext &context, Request &req) {
    return Resource::Delete(context, req); 
}

int SellerCommentList::Patch(const ResourceContext &context, Request &req) { 
